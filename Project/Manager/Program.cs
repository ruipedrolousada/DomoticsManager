...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine("4️⃣  View Help (Error Codes)");
    ...
    public static void PrintErrorTable()
    {
        // Header
        Console.WriteLine("╔════════════╦═════════════════════════╗");
        Console.WriteLine("║ Error Code ║ Error Name              ║");
        Console.WriteLine("╠════════════╬═════════════════════════╣");

        // Loop through the ErrorCode enum and print each error code and name
        foreach (var error in Enum.GetValues(typeof(LsnmpErrors.ErrorCode)).Cast<LsnmpErrors.ErrorCode>())
        {
            Console.WriteLine($"║ {((int)error),-11}║ {error,-24}║");
        }
    ...
    private static void HandleGetRequest(Manager manager)
    {
        ...
        // Validate the IID input
        for (int i = 0; i < iids.Length; i++)
        {
            if (!HelperMethods.IsValidIID(iids[i]))
            {
                Console.WriteLine($"❌ Invalid IID format: [{iids[i]}]. It must start with have atleast <structure.object> (e.g., '2.x').");
                return;
            }
        }
    ...
    private static void DisplayHelp()
    {
        ...
        Console.WriteLine("4️⃣ Special Cases (a.N.X.Z)");
        Console.WriteLine("   - Object = 0 → Returns the number of attributes in group/table");
        Console.WriteLine("   - X = 0 → Returns the total number of sensors/actuators");
        Console.WriteLine("   - X = 0 & Z = 0 → Returns all the values for N attribute in the table");
        Console.WriteLine("══════════════════════════════════════════════════════════════════════════════════════════\n");
    }
