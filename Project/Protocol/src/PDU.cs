...
public class PDU
{
    ...
    public static PDU Decode(string encodedPdu)
    {
        ...
        {
            throw new DecodingException($"[PDU] Error decoding PDU, {ex.Message}\nStack Trace: {ex.StackTrace}");
        }
    }
    ...
    public override string ToString()
    {
        ...
        sb.AppendLine("📦 **PDU Details**");
        sb.AppendLine("═══════════════════════════════════════════════════════════");
        sb.AppendLine($"🆔 Type:         {Type}");
        sb.AppendLine($"⏳ TimeStamp:    {TimeStamp}");
        sb.AppendLine($"🔑 Message ID:   {MessageIdentifier}");
        sb.AppendLine($"📌 **IID List:**    {(IIDList.Count > 0 ? string.Join(", ", IIDList) : "[Empty]")}");
        sb.AppendLine($"📊 **Value List:**  {(ValueList.Count > 0 ? string.Join(", ", ValueList) : "[Empty]")}");
        sb.AppendLine($"⚠️ **Error List:**   {(ErrorList.Count > 0 ? string.Join(", ", ErrorList) : "[Empty]")}");
